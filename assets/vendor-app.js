(function(global){
  ...
  function renderAuthGate(){
    ...
    document.getElementById('app').innerHTML = `<main class="auth-screen"><section class="auth-card">
      <div class="auth-copy">
        <span class="pill ok">GUN live vendor workspace</span>
        <h1>OMNI Vendor</h1>
        <p>Run sales, products, employee attendance, credit accounts, and orders from one synced vendor account.</p>
        <div class="auth-points"><span>Owner approval</span><span>Employee roles</span><span>POS ready</span></div>
      </div>
      <div class="auth-form">
  ...
  function renderPos(){
    ...
    const products = mine('products').filter(p=>p.active!==false && matchesTerm(p, ['name','category','barcode','sku','description']));
  ...
  function renderOrders(){
    ...
    document.getElementById('orders').innerHTML = `<div class="card pad"><div class="head"><h2>Orders</h2><span class="pill">${orders.length} records</span></div>${UI.table(orders, [{key:'id',label:'Order'}, {key:'customerName',label:'Customer'}, {key:'customerPhone',label:'Phone'}, {key:'customerAddress',label:'Address'}, {key:'paymentMethod',label:'Payment'}, {key:'status',label:'Status'}, {key:'total',label:'Total',format:r=>U.money(r.total)}], row=>canUpdate?`<button class="btn small" data-status="${row.id}" data-next="preparing">Preparing</button> <button class="btn small primary" data-status="${row.id}" data-next="done">Done</button>`:'')}</div>`;
  ...
  function renderCustomers(){
    ...
    const canWrite = can('credit.create') || can('credit.*');
    const canPay = can('credit.payment') || can('credit.*');
    document.getElementById('customers').innerHTML = `<div class="grid split"><div class="card pad"><div class="head"><h2>Credit Accounts</h2><span class="pill">${accounts.length} records</span></div>${UI.table(accounts, [{key:'phone',label:'Phone'}, {key:'customerName',label:'Customer'}, {key:'status',label:'Status'}, {key:'creditLimit',label:'Limit',format:r=>U.money(r.creditLimit)}, {key:'balance',label:'Balance',format:r=>U.money(r.balance)}, {key:'dueDays',label:'Due days'}], row=>canPay?`<button class="btn small" data-pay-credit="${row.id}">Record payment</button>`:'')}</div>${canWrite?`<form id="creditForm" class="card pad form"><h2>Add credit account</h2><div class="form-grid"><div class="field"><label>Customer name</label><input id="creditName"></div><div class="field"><label>Phone</label><input id="creditPhone" required></div><div class="field"><label>Limit</label><input id="creditLimit" type="number" min="0" step="0.001" value="50"></div><div class="field"><label>Due days</label><input id="creditDue" type="number" min="1" step="1" value="30"></div></div><button class="btn primary">Save credit account</button></form>`:permissionGate('This role can read credit records but cannot create accounts.')}<div class="card pad"><h2>Ledger</h2>${UI.table(ledger, [{key:'type',label:'Type'}, {key:'amount',label:'Amount',format:r=>U.money(r.amount)}, {key:'note',label:'Note'}])}</div></div>`;
    if(canWrite) {
  ...
  function startApp(){
    ...
    document.getElementById('logoutBtn').onclick = () => { global.OmniAuth.clearSession(); currentUser = null; POS.clear(); renderAuthGate(); };
